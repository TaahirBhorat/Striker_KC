...
import numpy as np
# Load your data
file_path = 'data/merged_psl_throughballs_data.csv'
...
# Sample players to plot
sample_players = ['Iqraam Rayners', 'Ashley Du Preez']

# Streamlit app
def main():
    st.title("Player Performance Comparison 2022/23")
    ...
    for player in df_ordered['player_name'].unique():
        if player not in sample_players:  # Skip highlighted players
    ...
    for player in sample_players:
        ...
        fig.add_trace(go.Scatter(x=std_from_mean.values, y=std_from_mean.index,
                                 mode='markers', marker=dict(color='blue' if player == 'Ashley Du Preez' else 'red', size=15),
                                 name=player, text=[player]*len(std_from_mean),
                                 hoverinfo='text'))
